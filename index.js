...
const spotifyRequest = (method, url) =>
  axios({ method, url, headers: { Authorization: `Bearer ${spotify.access_token}` } });
...
app.post("/vote", async (req, res) => {
  ...
  let downvotes = song.downvotes + (vote === "down" ? 1 : 0);
  let upvotes = song.upvotes + (vote === "up" ? 1 : 0);

  await supabase.from("current_song")
